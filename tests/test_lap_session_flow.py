...
@pytest.mark.asyncio
async def test_parser_callback_records_owned_porsche_boundary_after_stale_graphics(tmp_path):
    """The parser and lifecycle callback retain a log-owned Cup capture."""
    car_id = "00000000-0000-4000-8000-000000000001"
    log_file = tmp_path / "porsche-callback.log"
    log_file.write_text(
        "[2026-09-10 21:29:59.000] [network] [info] "
        f"76561198000000001 connected on car ks_porsche_992_gt3_cup, "
        f"with new carId {car_id}\n"
        "[2026-09-10 21:29:59.500] [gameplay] [info] "
        "Game Started! GameModeType_PRACTICE | Road Atlanta | "
        "ks_porsche_992_gt3_cup | GameModeSelectionWeatherType_Clear\n",
        encoding="utf-8",
    )
    ...
    capture = TelemetryCapture(
        output_dir=str(tmp_path), hz=20.0, session_manager=manager, record_frames=True
    )
    readers = {
    ...
    def decode_graphics(_raw):
        ...
        return {
            "car_model": (
                "Porsche 911 GT3 RS (992)" if sample_count == 1
                else "Porsche 911 GT3 Cup (992)"
            ),
            "status_name": "AC_LIVE",
            "session_phase": "Session",
            "current_lap_time_ms": 12_000,
    ...
    async def on_lap(parser_session, lap):
    ...
    lifecycle = SessionLifecycleService(
        ...
        stop_capture=capture.stop_capture,
    )
    ...
    with (
        ...
        try:
            ...
            while not capture.is_capturing() and asyncio.get_running_loop().time() < deadline:
                await asyncio.sleep(0.01)
            assert capture.is_capturing()
            with log_file.open("a", encoding="utf-8") as handle:
                handle.write(
                    "[2026-09-10 21:30:00.000] [gameplay] [info] Outplap split\n"
                    "[2026-09-10 21:30:00.000] [gameplay] [info] "
                    f"New lap carId {car_id}: 00:55.000\n"
                    "[2026-09-10 21:31:00.000] [gameplay] [info] "
                    f"New lap carId {car_id}: 01:00.000\n"
                )
            deadline = asyncio.get_running_loop().time() + 1.0
            while (
                parser.current_session is None
                or len(parser.current_session.laps) < 2
            ) and asyncio.get_running_loop().time() < deadline:
                await asyncio.sleep(0.01)
            assert parser.current_session is not None
            assert len(parser.current_session.laps) == 2
            assert len(history) == 1
        finally:
            parser.stop()
            await asyncio.wait_for(task, timeout=1.0)
            await capture.stop_capture("test")

    session = parser.current_session
    assert session is not None
    assert session.car == "ks_porsche_992_gt3_cup"
    assert session.car_uuid == car_id
    assert session.player_id == "76561198000000001"
    assert [lap.lap_time_ms for lap in session.laps] == [55_000, 60_000]
    assert [lap.lap_type for lap in session.laps] == ["OUTLAP", "VALID"]
    assert len(history) == 1
    assert manager.get_active_session_id() == session.session_id
    assert sample_count > 1
    assert manager.get_session_origin().graphics_car_model == "Porsche 911 GT3 Cup (992)"
...
async def test_restart_keeps_parser_owner_and_queued_completion() -> None:
    ...
    old_epoch = manager.get_session_origin().epoch
    ...
    assert manager.get_active_session_id() == parser.current_session.session_id
    assert manager.get_session_origin().epoch == old_epoch + 1
