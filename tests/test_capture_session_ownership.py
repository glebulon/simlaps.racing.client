...
from src.models.shared_session import car_models_match
...
@pytest.mark.asyncio
async def test_origin_rejection_diagnostics_are_bounded():
    manager = SharedSessionManager()
    manager.begin_session("session-bmw", car_model="BMW")
    capture = _capture_with_owner(manager=manager)
    ...
    with (
        patch("src.core.telemetry_capture.log_debug") as debug,
        patch("src.core.telemetry_capture.log_warning") as warning,
    ):
        await _run_samples(capture, sample)

    assert capture._origin_rejection_count == 10
    assert [call.kwargs["rejection_count"] for call in debug.call_args_list] == [1]
    assert [call.kwargs["rejection_count"] for call in warning.call_args_list] == [10]
...
def test_anonymous_capture_does_not_resume_after_mismatched_parser_epoch():
    ...
    manager.begin_session("session-bmw", car_model="BMW")
    assert manager.get_session_origin().epoch != original_epoch
    manager.update_from_graphics_shm(_live_graphics("BMW"))
...
PORSCHE_LOG_DISPLAY_PAIRS = (
    ("ks_porsche_992_gt3_cup", "Porsche 911 GT3 Cup (992)"),
    ("ks_porsche_992_gt3_rs", "Porsche 911 GT3 RS (992)"),
    ("ks_porsche_718_cayman_gt4_cs_mr", "Porsche 718 Cayman GT4 Clubsport"),
)


@pytest.mark.parametrize("log_car, display_car", PORSCHE_LOG_DISPLAY_PAIRS)
def test_verified_porsche_aliases_remain_strict_against_other_cars(log_car, display_car):
    assert car_models_match(log_car, display_car)
    other_car = "Porsche 911 GT3 Cup (992)" if "cup" not in log_car else "Porsche 911 GT3 RS (992)"
    assert not car_models_match(log_car, other_car)


@pytest.mark.asyncio
async def test_porsche_stale_rs_to_cup_capture_keeps_timed_lap_and_static_physics(tmp_path):
    """A stale RS mapping must not roll over a parser-owned Cup capture."""
    manager = SharedSessionManager()
    manager.update_from_graphics_shm(_live_graphics("Porsche 911 GT3 RS (992)"))
    manager.begin_session(
        "cup-session",
        car_model="ks_porsche_992_gt3_cup",
        car_uuid="cup-uuid",
    )
    capture = TelemetryCapture(
        output_dir=str(tmp_path),
    ...
    def graphics_sample(_raw):
        nonlocal decode_count
        if decode_count == 0:
            result = _live_graphics("Porsche 911 GT3 RS (992)")
        elif decode_count == 1:
            result = _live_graphics("Porsche 911 GT3 RS (992)")
        elif decode_count == 2:
            result = _live_graphics("Porsche 911 GT3 Cup (992)", current_lap_time_ms=0)
        elif decode_count == 3:
            result = _live_graphics("Porsche 911 GT3 Cup (992)", current_lap_time_ms=12_000)
            capture.record_lap_boundary(55_000, 1, "OUTLAP")
        elif decode_count == 4:
            result = _live_graphics("Porsche 911 GT3 Cup (992)", current_lap_time_ms=0)
        elif decode_count == 5:
            result = _live_graphics("Porsche 911 GT3 Cup (992)", current_lap_time_ms=10_000)
        else:
            result = _live_graphics("Porsche 911 GT3 Cup (992)", current_lap_time_ms=20_000)
            if decode_count == 6:
                capture.record_lap_boundary(60_000, 2, "VALID")
            if decode_count == 7:
                capture._running = False
    ...
    with (
        ...
        patch(
            "src.core.telemetry_capture.decode_static",
            return_value={"track": "Road Atlanta", "session_name": "PRACTICE"},
        ),
        patch("src.core.telemetry_capture.decode_physics", return_value={"speed_kmh": 180.0}),
    ):
        assert await capture.start_capture() is True
        await capture._task
        await capture.stop_capture("manual")

    assert manager.get_active_session_id() == "cup-session"
    assert manager.get_session_origin().epoch == capture.get_capture_origin().epoch
    assert manager.get_session_metadata_data().track == "Road Atlanta"
    assert manager._session_data.max_speed == 180.0
    assert [frame.frame_number for frame in capture.get_frames()] == [3, 4, 5, 6, 7]
    assert [(b.lap_time_ms, b.lap_number, b.lap_type) for b in capture.get_lap_boundaries()] == [
        (60_000, 2, "VALID")
    ]
    # The two stale RS samples and the Cup transition frame are discarded;
    # every stable Cup sample after rebinding is retained.
    assert capture._origin_rejection_count == 3


@pytest.mark.parametrize("log_car, display_car", PORSCHE_LOG_DISPLAY_PAIRS)
def test_porsche_alias_keeps_validity_only_live_timing(tmp_path, log_car, display_car):
    manager = SharedSessionManager()
    manager.begin_session("porsche-session", car_model=log_car, car_uuid="porsche-uuid")
    capture = _capture_with_owner(manager=manager, record_frames=False)
    capture._readers = {
        name: MagicMock(size=1, read_raw=MagicMock(return_value=b"x"))
        for name in ("graphics", "static", "physics")
    }
    graphics = _live_graphics(display_car, current_lap_time_ms=12_000)
    graphics["is_valid_lap"] = False
    with (
        patch("src.core.telemetry_capture.peek_graphics_validity", return_value={}),
        patch("src.core.telemetry_capture.decode_graphics", return_value=graphics),
        patch("src.core.telemetry_capture.decode_static", return_value={"track": "Road Atlanta"}),
        patch("src.core.telemetry_capture.decode_physics", return_value={"speed_kmh": 100.0}),
    ):
        frame = capture._capture_frame(0)

    assert frame.car_model == display_car
    assert capture._last_frame_origin_stable is True
    assert capture.get_frames() == []
    assert manager.get_current_lap_time() == 12_000
