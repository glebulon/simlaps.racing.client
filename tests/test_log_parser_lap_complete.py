...
class TestHandleLapCompleteBasic:
    ...
    def test_handle_lap_complete_returns_none_no_new_lap(self):
        ...
        parser = LogParser()
        parser.current_session = SessionData(track="spa", car="porsche", player_id="123")
...
class TestHandleLapCompleteWithData:
    ...
    def test_handle_lap_complete_invalid_lap(self):
        ...
        line = "[2024-01-01 12:00:00.000] [gameplay] [info] New lap carId abc123def4567890: 1:38.456"
        parser._handle_lap_complete(line)
    ...
    def test_handle_lap_complete_first_race_lap_grid_start_valid(self):
        ...
        line_new_lap = (
            "[2026-04-26 00:08:16.599] [gameplay] [info] New lap carId 48734ba531b55222-50fea70206fd60a7: 02:26.939"
        )
    ...
    def test_handle_lap_validity_flags_one_invalidates_valid(self):
        ...
        line_new_lap = "[2026-04-26 00:10:41.450] [gameplay] [info] New lap carId abc123-def456: 02:05.000"
        result = parser._handle_lap_complete(line_new_lap)
        assert result is None  # deferred emit
        assert parser._pending_lap is not None
        assert parser._pending_lap.lap_state == LapState.VALID
    ...
    def test_handle_lap_validity_flags_two_valid_even_when_boolean_false(self):
        ...
        line_new_lap = (
            "[2026-06-03 23:17:00.127] [gameplay] [info] New lap carId 42c664e0cfa01f9e-868df28c1b9fb49b: 01:10.548"
        )
        result = parser._handle_lap_complete(line_new_lap)
        assert result is None
        assert parser._pending_lap is not None
    ...
    def test_handle_lap_validity_accepts_flexible_field_order(self):
        ...
        parser._handle_lap_complete(
            "[2026-04-26 00:10:41.450] [gameplay] [info] New lap carId abc123-def456: 02:05.000"
        )
    ...
    def test_penalty_warning_before_lap_completion_demotes_to_invalid_penalty(self):
        ...
        parser._process_line(
            "[2026-08-13 22:24:23.707] [gameplay] [info] Penalty Type PenaltyType_Warning has no tranformation"
        )
        parser._handle_lap_complete(
            "[2026-08-13 22:24:28.867] [gameplay] [info] New lap carId abc123-def456: 02:04.000"
        )
    ...
    def test_penalty_added_deadline_duplicate_does_not_bleed_onto_next_lap(self):
        ...
        parser._process_line("[2026-08-18 23:13:02.935] [gameface] [warning] true {PENALTY_ADDED_KEY} #0 ")
        parser._handle_lap_complete(
            "[2026-08-18 23:13:03.050] [gameplay] [info] New lap carId abc123-def456: 03:01.000"
        )
        assert parser._pending_lap is not None
        assert parser._pending_lap.lap_state == LapState.INVALID_PENALTY
        ...
        parser._process_line("[2026-08-18 23:13:13.856] [gameface] [warning] true {PENALTY_ADDED_KEY} #0 ")
        assert parser._pending_penalty_warning is False

        # A genuinely new penalty >15s later still applies.
        parser._process_line("[2026-08-18 23:13:30.000] [gameface] [warning] true {PENALTY_ADDED_KEY} #0 ")
        assert parser._pending_penalty_warning is True
    ...
    def test_handle_lap_validity_upgrades_stale_penalty(self):
        ...
        line_new_lap = (
            "[2026-06-06 22:09:00.622] [gameplay] [info] New lap carId 4d19cc73858c594e-1fb0a71f9cba54a7: 06:18.351"
        )
        result = parser._handle_lap_complete(line_new_lap)
        assert result is None  # deferred emit
        assert parser._pending_lap is not None
        ...
        parser._process_line(
            "[2026-06-06 22:09:00.630] [gameplay] [info] Penalty Type PenaltyType_Warning has no tranformation"
        )
        assert parser._pending_lap.lap_state == LapState.INVALID_PENALTY
        assert parser._pending_lap.is_valid is False
    ...
    def test_handle_lap_validity_keeps_genuinely_invalid_penalty(self):
        ...
        parser._handle_lap_complete(
            "[2026-06-06 19:27:36.422] [gameplay] [info] New lap carId 4422a48dcd944a5a-d38c24fbe4c806a7: 06:25.308"
        )
        result = parser._handle_lap_validity(
...
@pytest.mark.parametrize("delta, expected", [(1, True), (-1, True), (3, False), (-3, False)])
