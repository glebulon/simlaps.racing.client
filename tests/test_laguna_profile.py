...
from src.core.telemetry_analyzer import TelemetryAnalyzer, _detect_profiled_corners_canonical
...
def test_laguna_profile_is_marked_estimated_until_real_replay_verifies_windows() -> None:
    profile = build_track_profile("laguna_seca", "full")

    assert profile["confidence"] == "estimated"
    assert {corner["_confidence"] for corner in profile["corners"]} == {"estimated"}


@pytest.mark.asyncio
async def test_laguna_estimated_status_reaches_generated_coaching_prompt(tmp_path) -> None:
    frames = []
    ...
    result = await TelemetryAnalyzer(output_dir=str(tmp_path)).analyze(
        ...
        output_prefix="laguna_estimated",
    )

    assert result.ai_prompt_path is not None
    prompt = (tmp_path / "telemetry_laguna_estimated_ai_prompt.txt").read_text(
        encoding="utf-8"
    )
    assert "profile" in prompt.lower()
    assert "estimated" in prompt.lower()
    assert "directional only" in prompt.lower()
